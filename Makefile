...
lint::
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' -m py_compile '$(REPOSITORY_ROOT_LITERAL)/scripts/check_watchos_contracts.py'

test::
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/check_watchos_contracts.py'
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/test_heart_animation_generation_contract.py'
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/test_heart_rate_query_ownership_contract.py'
...
build::
	@if [ -x '$(REPOSITORY_XCODEBUILD_LITERAL)' ]; then \
		'$(REPOSITORY_XCODEBUILD_LITERAL)' -project '$(REPOSITORY_ROOT_LITERAL)/HeartyMonitor.xcodeproj' -scheme HeartyMonitor -configuration Debug CODE_SIGNING_ALLOWED=NO build; \
	else \
